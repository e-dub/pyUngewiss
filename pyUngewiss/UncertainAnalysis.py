...
__title__ = ("Python library for UNcertainty analysis in liGhtwEight desiGn " +
...
class UncertainAnalysis(object):
    ...
    def calculate(self):
        ...
        def DefineProb(x0min, x0max, xL, xU, OptModel, DesVarNorm, ):
            if DesVarNorm == 1:
                ...
                MinProb = pyOpt.Optimization(OptModel, MinSysEqNorm,
                                             obj_set=None)
                MaxProb = pyOpt.Optimization(OptModel, MaxSysEqNorm,
                                             obj_set=None)
                if np.size(x0min) == 1:
                    MinProb.addVar('x', 'c', value=x0minnorm, lower=0.,
                                   upper=1.)
                    MaxProb.addVar('x', 'c', value=x0maxnorm, lower=0.,
                                   upper=1.)
                    self.nx = 1
                elif np.size(x0min) > 1:
                    for ii in range(np.size(x0min)):
                        MinProb.addVar('x'+str(ii+1), 'c', value=x0minnorm[ii],
                                       lower=0., upper=1.)
                        MaxProb.addVar('x'+str(ii+1), 'c', value=x0maxnorm[ii],
                                       lower=0., upper=1.)
                    self.nx = ii+1
            elif DesVarNorm == 0:
                ...
                if np.size(x0min) == 1:
                    ...
                    for ii in range(np.size(x0min)):
                        MinProb.addVar('x'+str(ii+1), 'c', value=x0min[ii],
                                       lower=xL[ii], upper=xU[ii])
                        MaxProb.addVar('x'+str(ii+1), 'c', value=x0max[ii],
                                       lower=xL[ii], upper=xU[ii])
                    self.nx = ii+1
            MinProb.addObj('f')
            MaxProb.addObj('f')
        ...
        def AlphaLevelOpt(OptProb, OptAlg, SensCalc, Name):
            if self.Alg in ["MMA", "GCMMA", "CONMIN", "KSOPT", "SLSQP", "PSQP",
                            "KSOPT", "SOLVOPT", "ALGENCAN", "NLPQLP"]:
                if SensCalc == "OptSensEq":
                    if self.paraNorm:
                        if Name[-3:] == "Min":
                            fOpt, xOpt, info = OptAlg(OptProb,
                    ...
                    fOpt, xOpt, info = OptAlg(OptProb, sens_type=SensCalc,
        ...
        for ir in range(self.nr):
            for ialpha in reversed(range(self.nAlpha)):
                ...
                if abs(np.sum(abs(xU-xL)/(xL+np.spacing(1)))) < 0.001:
                    ...
                    if self.Alg == "CMAES":
                        ResMin = cma.CMAEvolutionStrategy(x0min,  0.5,
                                                          {'bounds': [xL, xU]}).optimize(MinSysEq,
                                                                                         min_iterations=25,
                                                                                         iterations=100,
                                                                                         verb_disp=0)
                        ResMax = cma.CMAEvolutionStrategy(x0max, 0.5,
                                                          {'bounds': [xL, xU]}).optimize(MaxSysEq,
                                                                                         min_iterations=25,
                                                                                         iterations=100,
                                                                                         verb_disp=0)
                        rMin = ResMin[1]
                        ...
                        alphaLevelOptAlg = AlgOptions(alphaLevelOptAlg,
                                                      self.Alg, self.epsStop)
                        alphaLevelOptAlg.setOption("maxoutiter", 10)
                        MinProb, MaxProb = DefineProb(x0min, x0max, xL, xU,
                                                      self.UncertainModel,
                                                      self.paraNorm)
                        rMin, pMin = AlphaLevelOpt(MinProb, alphaLevelOptAlg,
                                                   self.SensCalc, Name+"Min")
                        rMax, pMax = AlphaLevelOpt(MaxProb, alphaLevelOptAlg,
                                                   self.SensCalc, Name+"Max")
                        self.Alg = "NLPQLP"
                        alphaLevelOptAlg = eval("pyOpt." + self.Alg + "()")
                        alphaLevelOptAlg = AlgOptions(alphaLevelOptAlg,
                                                      self.Alg, self.epsStop)
                        MinProb, MaxProb = DefineProb(pMin, pMax, xL, xU,
                                                      self.UncertainModel,
                                                      self.paraNorm)
                        rMin, pMin = AlphaLevelOpt(MinProb, alphaLevelOptAlg,
                                                   self.SensCalc, Name+"Min")
                        ...
                        if self.Alg == "PyGMO_monte_carlo":
                            algo = pg.algorithm(pg.monte_carlo(iters=AlgOptions.iter))
                        elif self.Alg == "PyGMO_de":
                            algo = pg.algorithm(pg.de(gen=ngen, F=1, CR=1,
                                                      variant=2, ftol=1e-03,
                                                      xtol=1e-03))
                        else:
                            algo = eval("pg.algorithm(pg." + self.Alg[6:] +
                                        '(' + str(AlgOptions.gen)+'))')
        ...
        if self.nr > 1:
            ...
            for i, val in enumerate(rUnc):
                if self.nAlpha == 1:
                    self.rUnc[i] = UncertainNumber(val, Form="interval",
                                                   nalpha=self.nAlpha)
                else:
                    self.rUnc[i] = UncertainNumber(val, Form="empirical",
                                                   nalpha=self.nAlpha)
        else:
            if self.nAlpha == 1:
                self.rUnc = UncertainNumber(rUnc[0][0], Form="interval",
                                            nalpha=self.nAlpha)
            else:
                self.rUnc = UncertainNumber(rUnc[0], Form="empirical",
                                            nalpha=self.nAlpha)
    ...
    def calcRobustness(self):
        ...
        self.SystemRobustnessNorm = np.divide(self.pAreaNormSum,
                                              self.rAreaNormSum)
...
if __name__ == "__main__":
    ...
    print("rUnc = \n" + str(Prob.rUnc.Value))
    print()
    print("Result with two-step methods stochastic and gradient-based algorithms:")
    Prob.Alg = "2Step"
    Prob.nEval = 0
    Prob.calculate()
    print("nEval = " + str(Prob.nEval))
