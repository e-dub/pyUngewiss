...
pgf_with_pdflatex = {"pgf.texsystem": "pdflatex",
...
class UncertainNumber(object):
    ...
    def calcArea(self):
        ...
        xVal = ([self.Value[:, 0], self.Value[:, 1]] +
                np.min(self.Value[:, :]))
    ...
    def plotValue(self, xlabel=[], ylabel=[], pdpi=100, fill=True,
        ...
        if self.Form == "interval" or self.nalpha == 1:   # for interval plot
            ...
            thick = 1**0.1*0.3
            yplaces = [.5+i for i in reversed(range(1))]
            ...
            pos = yplaces[0]
            ax.add_patch(patches.Rectangle((start, pos-delta/2.0), end-start,
                                           thick, color=color))
            ...
            if fill:
                ax.fill_between(self.Value[:, 0], mu, facecolor=color,
                                alpha=trans, linewidth=0.)
                ax.fill_between([self.Value[0, 1], self.Value[0, 0]], [1, 1],
                                facecolor=color, alpha=trans, linewidth=0.)
                ax.fill_between(self.Value[:, 1], mu, facecolor=color,
                                alpha=trans, linewidth=0.)
...
def plotIntervals(pUncList, labels=[],  Units=[], show=False, xPlot=5,
    ...
    yplaces = [.5+i for i in reversed(range(nP))]
    fig = plt.figure()
    ...
    if xAxisRot:
        plt.setp(ax.get_xticklabels(), rotation='vertical')
    plt.xlim(np.min(data)-(np.max(data)-np.min(data))/5,
...
def plotUncertainFn(pUncList, x=[], xlabel="Dependent parameter",
    ...
    if fill:
        for ii in reversed(range(np.size(rFuzz, 1)-1)):
            ax1.fill_between(x, rFuzz[:, ii+1, 1], rFuzz[:, ii, 1],
                             facecolor=color,
                             alpha=1./(nalpha)*(nalpha-ii-1)*.9, linewidth=0,
                             edgecolor=DefaultEdgeColor)
            ax1.fill_between(x, rFuzz[:, ii+1, 0], rFuzz[:, ii, 0],
                             facecolor=color,
                             alpha=1./(nalpha)*(nalpha-ii-1)*.9, linewidth=0,
                             edgecolor=DefaultEdgeColor)
        ax1.fill_between(x, rFuzz[:, 0, 1], rFuzz[:, 0, 0], facecolor=color,
                         alpha=1., linewidth=0)
    if outline:
        ax1.plot(x, rFuzz[:, :, 0], x, rFuzz[:, :, 1], color=color,
                 linewidth=1.)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    if ylimits != []:
